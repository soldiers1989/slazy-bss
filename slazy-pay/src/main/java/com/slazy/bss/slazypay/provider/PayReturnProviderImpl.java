...
/**
 * 
 * @Description: 退单记账接口实现类
 */
@Service("payReturnProvider")
public class PayReturnProviderImpl extends AbstractAccProvider implements BaseAccProvider<RechargeReqVo> {
	

	@Override
	public ResponseVo handle(RechargeReqVo vo) {
		log.info("开始处理退单记账！请求参数："+ JSON.toJSONString(vo));
	...
	public ResponseVo recharge(RechargeReqVo vo) throws Exception {
		...
		accountVo.setBizType(AccConst.BUSI_TYPE_PAY_CANCEL);  //19.订单取消退款
		...
		accountDetailVo.setRemark("订单取消退款记账！");
